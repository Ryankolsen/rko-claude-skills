...
**Scope each cycle's run to the file or seam you're testing, when `run-tests` supports it.** Read what the skill documents before assuming it can be scoped — some can only run the whole suite, and inventing a flag it does not support manufactures a false result. Where scoping exists, use it: the loop's speed is what makes red-green practical, and a full-suite run on every cycle both slows the loop and duplicates the one run `qa-verifier` is about to make independently. Where it does not, running the whole suite each cycle is simply what this project costs — do not skip cycles to compensate. Either way, you do not owe a final whole-suite run before reporting: that confirmation belongs to `qa-verifier`, and running it here a second time checks nothing a scoped loop and its own independent gate don't already cover.
